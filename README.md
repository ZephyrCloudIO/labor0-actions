...
jobs:
  labor0-agent-task:
    uses: ZephyrCloudIO/labor0-actions/.github/workflows/agent-task.yml@v1
    with:
...
The `v1` tag is the moving major tag for this runtime behavior. Keep `v0` pinned for wrappers that still need the original bootstrap-only runner contract.
