...
For graph-update planning tasks, the runner appends the manifest `graph_update_context` to the agent prompt, requests one YAML draft document, parses that YAML into the existing draft object, and reports a JSON `graph_update_draft` callback with the manifest graph head sequence. The callback and `graph_update_draft_path` file remain JSON for graph-agent compatibility. The parser keeps legacy JSON and Claude Code `structured_output` envelope fallback support, but the default runtime prompts no longer use JSON structured-output flags. For coding tasks, changed `read_write` repository checkouts are committed to a Labor0 branch, pushed, opened as pull requests by default, and reported back as `pull_request_linked` callbacks unless the binding sets `auto_pull_request_enabled=false`. When a repository manifest includes `pull_request_update` metadata, the runner commits and pushes changes to that existing pull request branch without creating or reporting a new pull request.

When a coding manifest enables `plan_mode`, `run-agent` first generates a provider-specific implementation plan, reports a `plan_proposed` callback, and blocks on the manifest `plan_decision_url` until graph-agent returns an approval, revision request, rejection, cancellation, or expiry. Codex planning uses `codex exec --ask-for-approval never --sandbox read-only`, Claude Code planning uses `--permission-mode plan`, and OpenCode planning runs without `--dangerously-skip-permissions`. Revision requests feed their notes into the next planning pass until the manifest revision limit is reached. Only an approved plan is appended to the implementation prompt; rejected, canceled, and expired plans fail the run before repository changes or pull requests are produced.
