...
For graph-update planning tasks, the runner appends the manifest `graph_update_context` to the agent prompt, requests one YAML draft document, parses that YAML into the existing draft object, and reports a JSON `graph_update_draft` callback with the manifest graph head sequence. The callback and `graph_update_draft_path` file remain JSON for graph-agent compatibility. The parser keeps legacy JSON and Claude Code `structured_output` envelope fallback support, but the default runtime prompts no longer use JSON structured-output flags. For coding tasks, the coding agent owns commits, pushes, and pull request creation. The runner exports `GH_TOKEN` from a writable repository token when available, preserves an explicitly supplied `GH_TOKEN`, exposes repository-specific tokens through `LABOR0_GITHUB_TOKENS_JSON`, records local branch names before implementation, compares them after a successful run, resolves pull requests for new local branches through `gh pr view`, and reports discovered pull requests as `pull_request_linked` callbacks. Repository bindings with `auto_pull_request_enabled=false` are included in the agent prompt as a no-PR policy; if a new pull request is still detected for that repository, the runner fails the task instead of reporting it. When a repository manifest includes `pull_request_update` metadata, the prompt instructs the agent to update that existing pull request branch and not create a new pull request; if that branch appears as a new local ref during the run, the runner treats it as the expected existing-PR update path instead of a new PR.
